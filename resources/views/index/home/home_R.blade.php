<!DOCTYPE html>
<html>
<head>
    <meta name="csrf-token" content="{{ csrf_token() }}">
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <meta charset="utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no" name="viewport" />
    <meta content="" name="description" />
    <meta content="" name="author" />
    <meta name="apple-mobile-web-app-capable" content="yes" />

    <title>測試專用頁面</title>

    <link href="./lib/css/buttons.css" rel="stylesheet" type="text/css"/>
    <link href="./lib/css/sweetalert.css" rel="stylesheet" type="text/css"/>
    <link href="./lib/css/font-awesome.min.css" rel="stylesheet" type="text/css"/>
    <link href="./lib/css/animate.css" rel="stylesheet" type="text/css"/>
    <link href="./lib/css/jquery.sidr.dark.min.css" rel="stylesheet" type="text/css"/>
    <link href="./css/randomBlock.css" rel="stylesheet" type="text/css"/>

    <!-- <link href="./lib/css/.css" rel="stylesheet" type="text/css"/> -->

    <script src="./lib/js/jquery-3.2.1.js" type="text/javascript"></script>
    <script src="./lib/js/jquery.cycle.all.min.js" type="text/javascript"></script>
    <script src="./lib/js/jquery.easing.1.3.js" type="text/javascript"></script>
    <script src="./lib/js/jquery.scrollTo.min.js" type="text/javascript"></script>
    <script src="./lib/js/jquery.sidr.min.js" type="text/javascript"></script>
    <script src="./lib/js/sweetalert.js" type="text/javascript"></script>
    <script src="./lib/js/base.js" type="text/javascript"></script>

    <script src="./js/randomBlock.js" type="text/javascript"></script>

    <!-- <script src="./lib/js/.js" type="text/javascript"></script> -->

    <script type="text/javascript">
        $(document).ready(function(){
            /** Alert **/
            {!! session()->get('msg', '') !!}
            {{ setMesage(null) }}
        });
    </script>
</head>
<body>
    <!-- box -->
    <div id="doSomethingBlock">
        <!-- top banner -->
        <header>
            <img src="./images/logo.png" height="80%">
            <div align="center"><h1>展示用網頁</h1></div>
            @if($box->loginType)
                <a href="" id="doSomethingBlockMenuButton">
                    <i class="fa fa-lg fa-bars" aria-hidden="true"></i>
                </a>
            @else
                <a href="/Login" id="doSomethingBlockLoginButton">
                    <i class="fa fa-lg fa-sign-in" aria-hidden="true"></i>
                </a>
            @endif
        </header>

        <div id="doSomethingBlockBanner">
            <div id="doSomethingBlockBannerBody">
                <img id="banner_1"/>
                <img id="banner_2"/>
                <img id="banner_3"/>
                {{-- <img src="images/ban4.jpg"/>
                <img src="images/ban5.jpg"/> --}}
            </div>
            <div id="doSomethingBlockBannerDown">
                <i class="fa fa-3x fa-chevron-down" aria-hidden="true"></i>
            </div>
        </div>


        <div id="doSomethingBlockBody">
            <div id="doSomethingBlockBodyBackImg" style="background-image: url(./images/ButtonBack.jpg)"></div>
            <div id="doSomethingBlockBodyFence"></div>
            <div id="doSomethingBlockBodyBox_0" class="BlockBodyBox">
                <img id="doSomethingBlockBodyBoxImg_0" src="./images/icon_1.png">
            </div>
            <div id="doSomethingBlockBodyBox_1" class="BlockBodyBox">
                <img id="doSomethingBlockBodyBoxImg_1" src="./images/icon_2.png">
            </div>
            <div id="doSomethingBlockBodyBox_2" class="BlockBodyBox">
                <img id="doSomethingBlockBodyBoxImg_2" src="./images/icon_3.png">
            </div>
            <div id="doSomethingBlockBodyBox_3" class="BlockBodyBox">
                <img id="doSomethingBlockBodyBoxImg_3" src="./images/icon_4.png">
            </div>
            <div id="doSomethingBlockBodyBox_4" class="BlockBodyBox">
                <img id="doSomethingBlockBodyBoxImg_4" src="./images/icon_5.png">
            </div>
        </div>
        <div id="doSomethingBlockBannerUp">
            <i class="fa fa-3x fa-chevron-up" aria-hidden="true"></i>
        </div>

    </div>
    <div id="sidr">
        <ul>
            <li>
                <div id="MenuMemberInfo">
                    <div id="MenuUserImg" style="background-image:
                        @if($box->loginType)
                            @if($box->member->photo != '')
                                url("$box->member->photo")
                            @else
                                url('data:image/jpeg;base64,')
                            @endif
                        @endif
                    "></div>
                    @if($box->loginType)
                        <span>暱稱：{{ $box->member->name }}</span>
                        <span>推薦碼：{{ $box->member->memberID }}</span>
                        <span>點數：{{ $box->member->points }}</span>
                        <span>藏蛋數：{{ $box->member->integral }}</span>
                        <span>贈送紅利：{{ $box->member->bonus }}</span>
                        <a href="/Logout">
                            <i class="fa fa-sign-out" aria-hidden="true"></i>
                            登出
                        </a>
                        <hr>
                    @endif
                </div>
            </li>
            <li><a href="/MFire">會員中心</a></li>
            <li><a>購物</a></li>
            <li><a>音樂</a></li>
            <li><a>教育</a></li>
            <li><a>遊戲</a></li>
            <li><a>直播</a></li>
            <li><a id="closeMenu">關閉選單</a></li>
        </ul>
    </div>
    <div id="backGatePage"></div>
</body>
</html>